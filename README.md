...
This creates the schema, saves a private backup under the ignored `.vercel/migration-backups/`, imports existing records in a transaction, and compares every imported field with the source. It never deletes the source. After the new deployment was verified, the old public `store/users.json` and `store/generations.json` were removed from Blob; the private backup remains in the ignored local migration directory.
