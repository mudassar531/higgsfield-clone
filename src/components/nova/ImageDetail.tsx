...
export default function ImageDetail({
  ...
  useEffect(() => {
    ...
    modal?.showModal();
    if (origin && panel.current && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      ...
      panel.current.animate(
        [
          { transform: `translate(${dx}px, ${dy}px) scale(${Math.max(0.1, origin.width / destination.width)}, ${Math.max(0.1, origin.height / destination.height)})`, opacity: 0.7 },
  ...
  function closeWithMotion() {
    ...
    if (!panel.current || !origin || window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      onClose();
      return;
    }
    const destination = panel.current.getBoundingClientRect();
    ...
    const dy = origin.top + origin.height / 2 - (destination.top + destination.height / 2);
    panel.current.animate(
      [{ transform: "translate(0, 0) scale(1)", opacity: 1 }, { transform: `translate(${dx}px, ${dy}px) scale(${Math.max(0.1, origin.width / destination.width)}, ${Math.max(0.1, origin.height / destination.height)})`, opacity: 0.5 }],
      { duration: 360, easing: "cubic-bezier(.6,0,.82,.2)" },
    ).finished.then(onClose).catch(onClose);
  }
  ...
  return (
    <dialog
      ...
      onCancel={(event) => { event.preventDefault(); closeWithMotion(); }}
      onClick={(event) => {
